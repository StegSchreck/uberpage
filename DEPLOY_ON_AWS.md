...
This is a little guide of how to deploy this React project to AWS using an AWS CodePipeline and a S3 Bucket. You still can deploy it in a different way. This is only a suggestion.

### Step 0: Create a domain
You will most likely want to access the React app through a proper domain (e.g. `yourdomain.tld`) or sub-domain (e.g. `uberpage.yourdomain.tld`) instead of the S3 url (e.g. `http://yourbucketname.s3-website.eu-central-1.amazonaws.com/`).
...
The S3 bucket will host your app.
  ...
  * Name the bucket like the url through which it will be accessed later, e.g. `yourdomain.tld` or `uberpage.yourdomain.tld`
      ...
      "Statement": [
        {
          ...
          "Principal": "*",
          "Action": "s3:GetObject",
          "Resource": "arn:aws:s3:::uberpage.yourdomain.tld/*"
        }
      ]
    }
    ```
    * Adjust the line `"Resource": "arn:aws:s3:::uberpage.yourdomain.tld/*"` with your domain
...
This will create an automated pipeline, which will deploy any changes you do to your repository. 
  ...
  * Click on "Create" and select a name for your pipeline, e.g. `UberPageDeployPipeline`
  ...
  * Select "create a new build project" and name it, e.g. `UberPageBuild`
  ...
  * Create a service role for your account, e.g. `code-build-UberPageBuild-service-role` (you can leave the generated value)
  ...
  * Add the permissions to that role to allow CodeBuild to push the build artifacts to the S3 bucket:
    * Open the "[IAM](https://console.aws.amazon.com/iam/home)" section in a new tab and edit the role, that you just created above (`code-build-UberPageBuild-service-role`)
  ...
  * Finally, you will have to create a AWS Service Role
    * Click "Create" and name it e.g. `AWS-CodePipeline-Service-UberPage`
    * Leave the default values for the other fields
  * In the next step you can review your settings
    * After creating the Pipeline, it will start to run automatically and your UberPage app will be deployed to your S3 bucket
...
For the domain, you created previously, adjust the settings in order point it to the S3 bucket
  ...
  * Add a `CNAME` record for the (sub-)domain you want to point to the UberPage app pointing to the S3 url
    * e.g. `http://uberpage.yourdomain.tld.s3-website.eu-central-1.amazonaws.com`
...
<p align="center">
  <img src="https://github.com/StegSchreck/UberPage/blob/master/src/assets/img/UberPage_AWS_CloudFront.png" width="435px">
</p>

You can configure a [AWS CloudFront](https://console.aws.amazon.com/cloudfront/home) distribution in order to offer your React website with HTTPS.
...
#### Step 3a: Create a free SSL certificate in AWS
  ...
  * Enter the domain name(s) you need, e.g. `yourdomain.tld` and `uberpage.yourdomain.tld`
...
#### Step 3b: Create the CloudFront distribution
  ...
  * Set the "Alternative Domain Names (CNAMEs)" to your domain, e.g. `uberpage.yourdomain.tld`
